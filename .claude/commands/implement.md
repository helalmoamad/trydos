...
- **IM-5 / GU-2** — if any planned file is under a high-risk path
  (`project-config.yaml > high_risk_paths`: auth/session & token storage,
  api/network clients (`lib/core/api/**`, `*_url_routes.dart`), composition root
  (`main.dart`, `lib/core/di/**`, `base_page.dart`, `service_provider.dart`,
  `lib/routes/**`), hydrated persisted state, calls & push notification services,
  wallet/order money surfaces, config/secrets, generated code), or the change
  alters authentication/session, wallet/order money integrity, hydrated BLoC state
  shape, or `main.dart` init order, or adds/renames a `ServerName`, or registers a
  NEW app-wide BLoC in DI + ServiceProvider, require `ticket.md > mode` =
  `high_risk`; else block. (A feature-local Bloc is normal standard work.)
- Branch checks:
  - *Initial (IM-3 / GU-4):* `dev_new` clean AND no `ticket/<slug>` branch exists.
...
- **Initial:** create and check out `ticket/<slug>` from clean `dev_new` (IM-3, the
...
- **IM-3** initial branch from clean dev_new (or **IM-3a** resume used the existing
