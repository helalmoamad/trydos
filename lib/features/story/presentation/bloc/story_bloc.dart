...
final GetWidthAndHeightUseCase getWidthAndHeightUseCase;
  StoryBloc(this.getStoryUseCase,this.getWidthAndHeightUseCase) : super(StoryState()) {
    on<StoryEvent>((event, emit){
    });
    on<GetStoryEvent>(_onGetStoryEvent,
        transformer: throttleDroppable(throttleDuration));
    on<LoadFailureEvent>(((event, emit) => emit(state.copyWith(getStoriesStatus: GetStoriesStatus.failure))));
    on<StorySelectedEvent>(_onStorySelectedEvent);

  }
  Future<void>_onStorySelectedEvent(StorySelectedEvent event,Emitter<StoryState>emit)
  async{
    emit(state.copyWith(getStoriesStatus: GetStoriesStatus.loading));
    final response= await getWidthAndHeightUseCase(widthAndHeightParams(url: state.stories[event.selected].stories![event.initialStory].photoPath!));

  response.fold((l) => emit(state.copyWith(getStoriesStatus: GetStoriesStatus.failure)), (r) => emit(state.copyWith(getStoriesStatus: GetStoriesStatus.success,initialStory: state.initialStory!+1)));
  }
