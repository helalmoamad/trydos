...
Map<String, List<Message>> groupReceivedMessageOnDays({
  ...
  chats.forEach((chat) {
    chat.messages?.removeWhere(
      (element) =>
          ((element.localId == null || element.localId == "null") &&
    ...
    for (int i = chat.messages!.length - 1; i >= 0; i--) {
      final zonedDate = HelperFunctions.replaceArabicNumber(
        DateFormat(
          "yyyy-MM-dd",
        ).format(HelperFunctions.getZonedDate(chat.messages![i].createdAt!)),
      );
      ...
      if (!newMessagesByDate.containsKey(zonedDate)) {
        ...
        newMessagesByDate[zonedDate]!.add(
          chat.messages![i].copyWith(isFirstMessageForThisDay: true),
        );
      } else
    ...
    for (String sendDate in newMessagesByDate.keys) {
      ...
      newMessagesByDate[sendDate]![0] = newMessagesByDate[sendDate]![0]
          .copyWith(isFirstMessage: true);
      for (int i = 1; i < newMessagesByDate[sendDate]!.length; i++) {
        if (newMessagesByDate[sendDate]![i - 1].senderUserId !=
          ...
          newMessagesByDate[sendDate]![i] = newMessagesByDate[sendDate]![i]
              .copyWith(isFirstMessage: true);
        }
      }
    }
