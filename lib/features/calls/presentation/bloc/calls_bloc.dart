...
import 'package:trydos/features/calls/domain/useCase/end_call_usecase.dart';
...
class CallsBloc extends Bloc<CallsEvent, CallsState> {
  ...
  FutureOr<void> _onResponseRejectVideoCallEvent(
    ResponseRejectVideoCallEvent event,
  ...
  FutureOr<void> _onGetMyCalls(
    ...
    response.fold(
      ...
      (r) {
        ...
        emit(
          state.copyWith(
            getMyCallsStatus: GetMyCallsStatus.success,
            callRegister: r.data,
          ),
        );
      },
  ...
  FutureOr<void> _onWatchMissedCallEvent(
    WatchMissedCallEvent event,
    Emitter<CallsState> emit,
  ) async {
    emit(
      state.copyWith(watchedMissedCallStatus: WatchedMissedCallStatus.loading),
    );
  ...
  FutureOr<void> _onGetMissedCallCountEvent(
    ...
    missedCallCount.fold(
      (l) => emit(
        state.copyWith(
          getMissedCallCountStatus: GetMissedCallCountStatus.failure,
        ),
      ),
      (r) {
        emit(
  ...
  FutureOr<void> _onInitResponseRejectVideoCallEvent(
    InitResponseRejectVideoCallEvent event,
    Emitter<CallsState> emit,
  ) async {
  ...
  FutureOr<void> _onDeleteMessageEvent(
    ...
    final response = await deleteMessageUseCase(
      DeleteMessageParams(
        messageId: event.messageId,
        deleteFromAll: event.deleteFromBoth,
      ),
    );
    response.fold(
      (l) {
        if (event.type == "message") {
          ...
          emit(state.copyWith(callRegister: callReg));
          emit(
            (state.copyWith(deleteMessageStatus: DeleteMessageStatus.failure)),
          );
        }
      },
      (r) {
        emit(
          (state.copyWith(deleteMessageStatus: DeleteMessageStatus.success)),
        );
      },
    );
  }
  ...
  FutureOr<void> _onUpdateCurrentActiveCallIdEvent(
    UpdateCurrentActiveCallIdEvent event,
    Emitter<CallsState> emit,
  ) {
