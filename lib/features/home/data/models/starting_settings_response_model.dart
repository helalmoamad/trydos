...
class Data {
  ...
  Data({this.startingSetting});

  Data copyWith({StartingSetting? startingSetting}) =>
      Data(startingSetting: startingSetting ?? this.startingSetting);
    ...
    return StartingSetting(
      notificationTypes: json["notificationTypes"] == null
          ...
          : List<NotificationType>.from(
              json["notificationTypes"]!.map(
                (x) => NotificationType.fromJson(x),
              ),
            ),
      decimalPointSettings: double.tryParse(
      ...
      orderGroupStatuses: json["order_group_statuses"] == null
          ...
          : List<OrderStatusModel>.from(
              json["order_group_statuses"]!.map(
                (x) => OrderStatusModel.fromJson(x),
              ),
            ),
      orderStatuses: json["order_statuses"] == null
          ? []
          : List<OrderStatusModel>.from(
              json["order_statuses"]!.map((x) => OrderStatusModel.fromJson(x)),
            ),
    );
  }
...
class OrderStatusModel {
  ...
  OrderStatusModel({this.value, this.label});

  factory OrderStatusModel.fromJson(Map<String, dynamic> json) =>
      OrderStatusModel(value: json["value"] ?? '', label: json["label"] ?? '');
...
class NotificationType {
  ...
  NotificationType copyWith({int? id, String? name}) =>
      NotificationType(id: id ?? this.id, name: name ?? this.name);

  factory NotificationType.fromJson(Map<String, dynamic> json) =>
      NotificationType(id: json["id"], name: json["name"]);
