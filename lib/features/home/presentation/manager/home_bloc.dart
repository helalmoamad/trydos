  ...
  FutureOr<void> _onGetProductsWithFiltersEvent(
    ...
    if (!(event.fromChoosed ?? false)) {
      if (((filters.colors?.isNullOrEmpty ?? true) &&
        ...
        List<String>? boutiques = event.fromSearch ?? false
        ...
        List<String>? prices =
            (prevAppliedFiltersByUser[key]?.filters?.prices?.maxPrice != null &&
                ...
                : null;
        List<String>? options = filters.attributes?[0].options;
        ...
        FirebaseAnalyticsService.logEventForSession(
          ...
          extraParams: {
            'brands': json.encode(brands ?? []),
            'categories': json.encode(categories ?? []),
            'boutiques': json.encode(boutiques ?? []),
            'colors': json.encode(colors ?? []),
            'prices': json.encode(prices ?? []),
            'options': json.encode(options ?? []),
            'searchText': json.encode(searchText ?? ''),
          },
        );
      }
    }
  ...
  FutureOr<void> _onAddItemToCartEvent(
    ...
    if (!CurrentQuantity[key].isNullOrEmpty) {
      ...
      if (quantity > (double.tryParse(event.maxAllowed ?? "0") ?? 0) &&
        ...
        print(
            "05555555555/8888888888888888888888888888888888888888888888888888888888888888");
      ...
      if (event.quantity!.round() >
          ...
          (double.tryParse(event.maxAllowed ?? "0") ?? 0) != 0) {
        print(
            "11111111111/9999999999999999999999988888888888888888888888888888888888888888888888888888888888888888888888888888");
    ...
    print(
        "2222222222/9999999999999999999999988888888888888888888888888888888888888888888888888888888888888888888888888888");
